	...
	return (
		<header
			className={`sticky z-50 w-full bg-white dark:bg-dark top-0 ${
				isTop ? `bg-opacity-100` : `bg-opacity-80 backdrop-blur-sm dark:bg-opacity-80`
			}`}
		>
			<div className=' box-screen px-4 mx-auto py-0'>
				<div className=' flex items-center justify-between -mx-1 relative'>
					<div className='w-[15rem] max-w-full px-4'>
						<img src='/images/logo-white.svg' alt='logo' className='lg:py-2 py-5 w-full dark:block hidden' />
						<img src='/images/logo.svg' alt='logo' className='lg:py-2 py-5 w-full block dark:hidden' />
					</div>
					<div className=' flex justify-end items-center px-4 w-full'>
						<div>
							...
							>
								<FaBars className={`text-2xl text-main ${isDropdownOpen.bar ? `hidden` : `block`}`} />
								<FaPlus className={`text-main text-2xl rotate-45 ${!isDropdownOpen.bar ? `hidden` : `block`}`} />
							</button>
							<nav
